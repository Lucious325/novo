...
fetch('data/MU_list.json')
  .then(r => r.json())
  .then(data => {
      MU_list = data;
      console.log('MU list loaded', MU_list);
      reloadDashboard(); 
  });
...
function groupByUnits(data){
    const map = {};
    const playersByUnit = {};
    data.forEach(d=>{
        if(!d.UNIT) return;
        const key = `${d.UNIT}-${d.ID}`;
        if(!playersByUnit[key]) playersByUnit[key] = [];
        playersByUnit[key].push(d);
    });

    Object.values(playersByUnit).forEach(arr=>{
        arr.sort((a,b)=> new Date(a.CollectedAt)-new Date(b.CollectedAt));
        const first = arr[0], last = arr[arr.length-1];
        const unitID = first.UNIT;
        const unitName = MU_list[unitID] || `Unit ${unitID}`;
        if(!map[unitID]) {
            map[unitID] = { UNIT: unitID, UnitName: unitName };
            sections.forEach(s=> map[unitID][s]=0);
        }
        sections.forEach(s=>{
            map[unitID][s] += (last[s] || 0) - (first[s] || 0);
        });
    });
...
function renderCountriesView() {
    ...
    sections.forEach(section => {
        ...
        cardList.innerHTML=''; pag.innerHTML='';
        cardHeader.textContent = `Top by ${section} per Country`;

        const page = currentPages[section] || 1;
        const sorted = grouped.sort((a,b)=>(b[section]||0)-(a[section]||0));

        paginate(sorted,page).forEach((c,i)=>{
            ...
            p.innerHTML = `
                ...
                <span style="flex: 1; display:flex; align-items:center; gap:8px;">
                    <img src="${flagURL}" style="width:26px; height:18px; object-fit:cover; border:1px solid #333;" onerror="this.style.display='none'">
                    <span>${c.CS}</span>
                </span>
        ...
        renderPagination(pag,page,sorted.length,p=>{currentPages[section]=p; renderCountriesView()});
    });
}
...
function renderUnitsView() {
    document.getElementById('homeContainer').style.display = 'grid';
    document.getElementById('citizenContainer').style.display = 'none';
    const grouped = groupByUnits(allData);
    ...
    sections.forEach(section => {
        ...
        cardList.innerHTML = '';
        pag.innerHTML = '';
        cardHeader.textContent = `Top by ${section} per Unit`;
        ...
        paginate(sorted, page).forEach((c, i) => {
            ...
            const imgURL = `https://edominations.com/public/upload/group/${c.UNIT}.webp`;
            ...
            p.innerHTML = `
                <span style="flex: 1; display:flex; align-items:center; gap:8px;">
                    <span style="width:20px; text-align:right;">${position}.</span>
                    <img src="${imgURL}" style="width:26px; height:26px; object-fit:cover; border:1px solid #333;" 
        ...
        renderPagination(pag, page, sorted.length, p => {
...
if(toggle){
    toggle.onchange = ()=>{
        if(toggle.checked){
            currentViewMode = 'countries';
            label.textContent = 'Countries';
        } else {
