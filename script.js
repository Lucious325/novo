...
let allData = [];
const currentPages = {}; // za paginaciju svake kartice
...
function renderSection(section, dataToUse){
    ...
    const globalIndex = (currentPage - 1) * perPage + index + 1; // rang u odnosu na sve

    const pElem = document.createElement('p');

    // SPOJEN rank + ime u jedan span → bez ikakvog razmaka
    const nameSpan = document.createElement('span');
    nameSpan.className = 'player-name';
    nameSpan.textContent = `${globalIndex}.${p.Name}`;
    nameSpan.style.cursor = 'pointer';

    // klik na ime otvara citizen view
