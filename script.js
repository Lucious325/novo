...
function groupByCountry(data){
    const map = {};
    const latestDate = new Date(Math.max(...data.map(d=>new Date(d.CollectedAt))));
    const startDate = new Date(latestDate);
    startDate.setDate(startDate.getDate()-30);

    const playersByCS = {};
    data.forEach(d=>{
        if(new Date(d.CollectedAt)<startDate) return;
        const key = `${d.CS}-${d.ID}`;
...
function renderSection(section,data){
    ...
    const pag = document.querySelector(`#${sectionToCardId[section]} .pagination`);
    list.innerHTML='';
    cardHeader.textContent = `Top 10 by ${section} Increase (Last 30 Days)`;

    const latest = new Date(Math.max(...data.map(d=>new Date(d.CollectedAt))));
    const start = new Date(latest); start.setDate(start.getDate()-30);
